...
type UniformMap = Record<string, { value: THREE.Vector3 | THREE.Vector4 | number }>;
...
export default function LaserFlow({
  ...
  useEffect(() => {
    ...
    const setSize = () => {
      ...
      (uniforms.iResolution.value as THREE.Vector3).set(w * pixelRatio, h * pixelRatio, pixelRatio);
    };
    ...
    const onMove = (ev: PointerEvent) => {
      ...
      const x = (ev.clientX - rect.left) * pixelRatio;
      const y = (rect.height - (ev.clientY - rect.top)) * pixelRatio;
      mouse.set(x, y);
    };
    ...
    const animate = () => {
      ...
      (uniforms.iMouse.value as THREE.Vector4).set(mouse.x, mouse.y, 0, 0);
